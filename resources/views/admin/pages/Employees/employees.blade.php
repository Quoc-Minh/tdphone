              ...
              <table class="table">
                <thead>
                  <tr>
                    ...
                    <th><button class="table-sort" data-sort="sort-role">{{ __('role') }}</button></th>
                    <th><button class="table-sort" data-sort="sort-email">{{ __('email') }}</button></th>
                    <th><button class="table-sort" data-sort="sort-phone">{{ __('phone number') }}</button></th>
                ...
                <tbody class="table-tbody">
                  ...
                  <tr>
                    ...
                    <td>
                      <a href="#" data-bs-toggle="modal" data-bs-target="#modal-edit_{{ $user->id }}">{{ __('Edit') }}</a>
                    </td>
                    @endcan
                    @can('thêm nhân viên')
                    <td>
                      <a href="/admin/employees/{{ $user->id }}/delete" data-confirm-delete="true">{{ __('Delete') }}</a>
                    </td>
                    @endcan
                  </tr>
                  @endforeach
    ...
    <div class="modal-content">
      ...
      <form action="{{ Route('admin.employees.update', ['id' => $user->id]) }}" method="POST">
        ...
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label required">{{ __('Name') }}</label>
            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" placeholder="{{ __('Enter name') }}..." value="{{ $user->ten }}">
          </div>
          <div class="mb-3">
            <label class="form-label required">Email</label>
            <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" placeholder="Enter email..." value="{{ $user->email }}">
            @error('email') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>
          <div class="mb-3">
            <label class="form-label required">{{ __('Phone') }}</label>
            <input type="text" class="form-control @error('phone') is-invalid @enderror" name="phone" placeholder="Enter phone..." value="{{ $user->sodienthoai }}">
            @error('phone') <div class="invalid-feedback">{{ $message }}</div> @enderror
          </div>
          <div class="mb-3">
            <label class="form-label required">Password</label>
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    const list = new List('table-roles', {
      ...
      valueNames: ['sort-number', 'sort-name', 'sort-role', 'sort-email', 'sort-phone', 'sort-address', {
        attr: 'data-date',
        name: 'sort-date'
      }]
    });
